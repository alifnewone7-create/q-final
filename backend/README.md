...
Config is in `.env` (BOT_TOKEN, ADMIN_ID, QUOTEX_EMAIL, QUOTEX_PASSWORD, ACCOUNT_TYPE, OWNER_TAG).

### Files
- `bot.py` — bot entry point (menus, channels, session control)
- `sessions.py` — signal engine (analyze → signal → MTG-1 result → partial)
- `qx.py` — Quotex connection/markets/candles (pyquotex)
- `analysis.py` — next-candle prediction logic
- `charting.py` — dark-theme candlestick chart PNG
- `storage.py` — JSON persistence (`data/channels.json`, `data/settings.json`, `data/signals.json`)
